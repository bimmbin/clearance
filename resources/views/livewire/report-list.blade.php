        ...
        <tbody class="text-left">
            @forelse ($reports as $report)
                <tr class="bg-tablebg border">
                    ...
                    <td class="py-2 pl-10">{{ $report->profiles->firstname . ' ' . $report->profiles->lastname }} has {{ $report->details }} {{ $report->clearance->profiles->firstname . ' ' . $report->clearance->profiles->lastname }} clearance</td>
                    <td class="py-2 pl-10">{{ $report->created_at->diffForHumans() }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="2" class="text-center py-5">
                        <div wire:loading.delay wire:target="loadReports" class="text-gray-500 font-medium">Loading...</div>
